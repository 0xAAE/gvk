...
pub fn build(application: &gtk::Application, rx: MessageReceiver, storage: Arc<Storage>) {
    ...
    // test stored auth
    let access_token_valid = if let Ok(auth) = storage.load_auth() {
        //todo: logging
        println!("use stored auth: {}", auth);
        //todo: test auth
        true
    } else {
        false
                ...
                if let Some(uri) = view.get_uri() {
                    if AccessTokenProvider::is_auth_succeeded_uri(uri.as_str()) {
                        ...
                        if let Ok(auth) = uri.as_str().parse::<AuthResponse>() {
                            ...
                            // sync storing
                            if let Err(what) = storage_ref.save_auth(&auth) {
                                println!("Failed saving auth: {}", what);
                            } else {
                                println!("Auth saved into {}", storage_ref.get_auth_file_name().as_str());
                            }
